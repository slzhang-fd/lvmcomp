...
double log_pos_theta_mirt(double x, void* params){
  ...
  arma::vec tmp = d_params->d;
  theta(d_params->k) =  x;
  tmp = d_params->A * theta + d_params->d;
  return -0.5 * arma::as_scalar(theta.t() * d_params->inv_sigma * theta) + 
    arma::accu(d_params->y_i % tmp - log(1+exp(tmp)));
}
// [[Rcpp::export]]
...
arma::vec sample_theta_i_arms_partial_credit(arma::vec theta0_i, arma::vec y_i,
  ...
  int neval;
  double xl = -100.0, xr = 100.0;
  double xinit[10]={-1.0, -0.5, 0.5, 1.0}, xsamp[100], xcent[10], qcent[10] = {5., 30., 70., 95.};
  double convex = 1.;
  int dometrop = 0;
  double xprev = 0.0;
